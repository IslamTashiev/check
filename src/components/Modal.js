...
const Modal = ({ selectedCard, setSelectedCard, setCheckProducts, setProducts }) => {
  const { price, count, unitOfMeasurement, title, image, id } = selectedCard;
  ...
  const increaseCount = () => {
    setCount((prevState) => parseInt(prevState.split(unitOfMeasurement)[0]) + 1 + unitOfMeasurement);
  };
  const decreaseCount = () => {
  ...
  const onCloseModal = () => {
    ...
    setCheckProducts((prevState) => [...(prevState ?? []), ...tableData]);
  };
